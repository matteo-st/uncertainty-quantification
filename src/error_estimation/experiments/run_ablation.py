...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run n_cal ablation experiments.")
    parser.add_argument(
        "--config-dataset",
        "--config_dataset",
        dest="config_dataset",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override calibration sample sizes (space-separated)",
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    logger.info("Starting n_cal ablation in %s", run_root)
    ...
    with tracker:
        ...
        n_cal_values = args.n_cal
        if n_cal_values is None:
            n_cal_cfg = data_cfg["n_samples"]["cal"]
            n_cal_values = n_cal_cfg if isinstance(n_cal_cfg, list) else [n_cal_cfg]

        logger.info("Seed splits: %s", seed_splits)
        logger.info("n_cal values: %s", n_cal_values)
        ...
        for seed_split in seed_splits:
            ...
            for n_cal in n_cal_values:
                run_dir = seed_dir / f"n-cal-{n_cal}" / f"seed-split-{seed_split}"
                ensure_dir(run_dir)
                ...
                with tracker.child_run(run_name=f"seed-split-{seed_split}_n-cal-{n_cal}", tags=child_tags):
                    ...
                    tracker.log_artifacts(
                        run_dir,
                        artifact_path=f"results/{run_tag}/variants/n-cal-{n_cal}/seed-split-{seed_split}",
                    )
