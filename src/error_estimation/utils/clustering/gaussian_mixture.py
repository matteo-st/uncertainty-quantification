...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self, x):
        ...
        if self.mu_init is not None:
            ...
            self.mu = torch.nn.Parameter(torch.randn(1, self.n_components, self.n_features, generator=gen, device=x.device), requires_grad=False)
    ...
    def fit(self, x, delta=1e-3, warm_start=False):
        """
        ...
        while (i <= self.max_iter) and (j >= delta):
