...
run_step1() {
    ...
    echo "Evaluates temperature grid (magnitude=0) on res, cal, and test splits"
    ...
    nohup python -m error_estimation.experiments.run_detection \
        ...
        --config-detection configs/postprocessors/msp/imagenet_timm-vit-tiny16.yml \
        --eval-grid \
        --run-tag "$RUN_TAG_BASELINE" \
        --results-family baselines \
        --seed-splits 1 2 3 4 5 6 7 8 9 \
        ...
        --checkpoints-dir "${CHECKPOINTS_DIR}" \
        --no-mlflow \
        > logs/msp_grid_imagenet_vit_tiny16.log 2>&1 &
    PID5=$!
    echo "    PID: $PID5"
    ...
    nohup python -m error_estimation.experiments.run_detection \
        ...
        --config-detection configs/postprocessors/msp/imagenet_timm-vit-base16.yml \
        --eval-grid \
        --run-tag "$RUN_TAG_BASELINE" \
        --results-family baselines \
        --seed-splits 1 2 3 4 5 6 7 8 9 \
        ...
        --checkpoints-dir "${CHECKPOINTS_DIR}" \
        --no-mlflow \
        > logs/msp_grid_imagenet_vit_base16.log 2>&1 &
    PID6=$!
    echo "    PID: $PID6"
    ...
    echo "  tail -f logs/msp_grid_imagenet_vit_tiny16.log"
    echo "  tail -f logs/msp_grid_imagenet_vit_base16.log"
}
...
run_step2() {
    ...
    nohup python -m error_estimation.experiments.run_detection \
        --config-dataset "$DATASET_IMAGENET" \
        --config-model configs/models/imagenet_timm-vit-tiny16.yml \
        --config-detection configs/postprocessors/partition/imagenet_vit_tiny16_msp-unif-mass.yml \
        --eval-grid \
        --run-tag "$RUN_TAG_BINNING" \
        --results-family partition_binning \
        --seed-splits 1 2 3 4 5 6 7 8 9 \
        ...
        --logits-dtype float64 \
        --no-mlflow \
        > logs/msp_unif_mass_grid_imagenet_vit_tiny16.log 2>&1 &
    PID5=$!
    echo "    PID: $PID5"
    ...
    nohup python -m error_estimation.experiments.run_detection \
        --config-dataset "$DATASET_IMAGENET" \
        --config-model configs/models/imagenet_timm-vit-base16.yml \
        --config-detection configs/postprocessors/partition/imagenet_vit_base16_msp-unif-mass.yml \
        --eval-grid \
        --run-tag "$RUN_TAG_BINNING" \
        --results-family partition_binning \
        --seed-splits 1 2 3 4 5 6 7 8 9 \
        ...
        --logits-dtype float64 \
        --no-mlflow \
        > logs/msp_unif_mass_grid_imagenet_vit_base16.log 2>&1 &
    PID6=$!
    echo "    PID: $PID6"
    ...
    echo "  tail -f logs/msp_unif_mass_grid_imagenet_vit_tiny16.log"
    echo "  tail -f logs/msp_unif_mass_grid_imagenet_vit_base16.log"
}
